...
2) **`current/`** — Authoritative **as-is** system, APIs, infra, indexes.  
3) **`contentful-docs/`** — Definitive platform docs for Contentful usage and taxonomy.  
4) *(Optional)* `target/` — Drafts and proposals when present.

> The goal is to implement `product/` within (and evolving) the current system described in `current/`, using `contentful-docs/` for platform constraints and best practices.

---
## 🧠 Grounding Order (how the GPT reasons)

1) **product/** (Requirements → Vision → Initiative)  
2) **current/** (compare/contrast with as-is)  
3) **contentful-docs/** (platform specifics & constraints)  
4) *(Optional)* target drafts
...
│  └─ contentful-app-framework-functions-guides.md
│
├─ current/                                  # As-is system, APIs, infra, indexes (definitive current state)
...
└─ target/
   ...
   ├─ contentful-taxonomy-integration-strategy.md
   └─ denormalized-hierarchical-indexing-igaming-search.md
...
- **OpenSearch schema & integration (as-is)**  
  `current/contentful-opensearch-schema-guide.md`  
...
- **Contentful modeling & taxonomy (platform docs)**  
  `current/contentful-cms-modeling-guide.md`  
...
- **Webhooks & runbook (as-is)**  
  `current/contentful-opensearch-webhooks-guide.md`  
  `current/contentful-opensearch-webhook-runbook.md`

- **Capability gaps/status (as-is)**  
  `current/current-search-capability-report.md`

- **Author new solution docs**  
  `target/000-template-solution-architecture.md`
